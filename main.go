...
func main() {
	...
	registerKaraokeRoutes(r, *dataDir, secureHeaders)
	...
	api.POST("/client-log", func(c *gin.Context) {
		...
		if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"invalid log body"}); return }
		entry := map[string]any{"server_time":time.Now().UTC().Format(time.RFC3339Nano), "remote_ip":c.ClientIP(), "user_agent":c.Request.UserAgent()}
		var payload any
		if json.Unmarshal(body, &payload) == nil { entry["client"] = payload } else { entry["client_raw"] = string(body) }
		line, _ := json.Marshal(entry)
		log.Printf("client-log %s", line)
		if f, ferr := os.OpenFile(filepath.Join(*dataDir, "client.log"), os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0o600); ferr == nil { _, _ = f.Write(append(line, '\n')); _ = f.Close() }
