...
function renderCrawlStatus(){
  ...
  el.innerHTML='来源：'+esc(catalog.provider)+'<br>最近抓取：'+esc(catalog.last_crawl_shanghai||'尚未完成')+'<br>下次：'+esc(catalog.next_crawl_shanghai||'-')+(catalog.last_error?'<br><span class="bad">'+esc(catalog.last_error)+'</span>':'');
}
